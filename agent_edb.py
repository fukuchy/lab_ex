"""
agent_edb.py - agent_final_ending.py に終盤データベース (EDB) 参照を追加したエージェント

agent_final_ending.py からの追加:
  EDB (終盤データベース) を endgame_search の先頭で参照し、
  事前計算済みの勝敗結果があればそのまま返すことで終盤読み切りを高速化する。

  EDB ファイルは generate_edb.py で事前生成しておく。
  キー: (player_bits, opponent_bits) の整数タプル  ← 実行をまたいで安定
  値  : WIN_SCORE / 0 / -WIN_SCORE (現在の手番側から見たスコア)

  設定項目:
    EDB_FILE         = "edb_6.pkl"  # 読み込む EDB ファイル名
    EDB_LOOKUP_LIMIT = 6            # EDB を参照する空きマス数の上限
                                    # generate_edb.py の --empty と合わせること
...
# ==============================
#
# generate_edb.py で事前生成した pkl ファイルを起動時に読み込み、
# endgame_search の先頭で参照することで完全読みを高速化する。
#
# キー: (player_bits, opponent_bits) の整数タプル
#   - player_bits : pos.side_to_move の石の 64bit ビットボード
#   - opponent_bits: 相手の石の 64bit ビットボード
#   - Python の hash() は実行ごとに変わるため不使用
#   - 整数ビット列はプログラム実行をまたいでも常に同じ値になる
#
# 値: WIN_SCORE / 0 / -WIN_SCORE (pos.side_to_move 視点のスコア)
#
# EDB_LOOKUP_LIMIT は generate_edb.py の --empty に合わせる。
# (例: --empty 6 で生成した場合は EDB_LOOKUP_LIMIT = 6)

EDB_FILE         = os.path.join(BASE_DIR, "edb_6.pkl")
EDB_LOOKUP_LIMIT = 6   # EDB を参照する空きマス数の上限


def _get_bitboards(pos) -> tuple:
    """
    局面から (player_bits, opponent_bits) のタプルを取得する。
    EDB のキーとして使用する。
    """
    p = o = 0
...
def _load_edb(filepath: str) -> dict:
    """
    EDB ファイルを読み込む。
    ファイルが存在しない・読み込み失敗の場合は空辞書を返し、
    EDB なしの通常動作にフォールバックする。
    """
    if not os.path.exists(filepath):
        print(f"[EDB] {filepath} が見つかりません。EDB なしで動作します。")
        return {}
    try:
        with open(filepath, "rb") as f:
            edb = pickle.load(f)
        print(f"[EDB] 読み込み完了: {len(edb):,} 局面  ({filepath})")
        return edb
    except Exception as e:
        print(f"[EDB] 読み込み失敗: {e}  EDB なしで動作します。")
        return {}


EDB: dict = _load_edb(EDB_FILE)
...
def _eval_one_empty(pos, ctx, ply):
    """
    ③ 残り1マスの局面を再帰なし・TT参照なしで直接評価する。

    endgame_search の最深部（最も多く呼ばれる）の関数呼び出しオーバーヘッドを
    完全に排除することで、終盤探索全体のノード処理速度を向上させる。

    局面 pos の side_to_move（現在の手番側）から見たスコアを返す。

    使用するバッファ:
        ctx.pos_pool[ply]  : パス後・または着手後の局面
        ctx.scratch_pos    : パス後に相手が着手する場合の局面
    """
    actions = list(pos.get_legal_moves())

    if actions:
        # 現在の手番側が合法手を持つ → 着手して終局
        action    = actions[0]          # 残り1マスなので合法手は最大1手
        flip_bits = pos.calc_flip_discs(action)
        next_pos  = ctx.pos_pool[ply]
        pos.copy_to(next_pos)
        next_pos.do_move(action, flip_bits)
        # do_move 後は side_to_move が相手に切り替わる
        #   next_pos.player_disc_count   = 相手の石数
        #   next_pos.opponent_disc_count = 自分(元の手番側)の石数
        my_cnt  = next_pos.opponent_disc_count
        opp_cnt = next_pos.player_disc_count
        if my_cnt > opp_cnt: return  WIN_SCORE
        if my_cnt < opp_cnt: return -WIN_SCORE
        return 0
    ...
    opp_actions = list(next_pos.get_legal_moves())

    if not opp_actions:
        # 双方ともパス不可 → 現在の石数で終局
        my_cnt  = pos.player_disc_count
        opp_cnt = pos.opponent_disc_count
        if my_cnt > opp_cnt: return  WIN_SCORE
        if my_cnt < opp_cnt: return -WIN_SCORE
        return 0

    # 相手が合法手を持つ → 相手が着手して終局
    # next_pos は ctx.pos_pool[ply] (パス後)、scratch_pos に着手結果を格納
    opp_action = opp_actions[0]
    flip_bits  = next_pos.calc_flip_discs(opp_action)
    next_pos.copy_to(ctx.scratch_pos)
    ctx.scratch_pos.do_move(opp_action, flip_bits)
    # 相手の do_move 後は side_to_move が自分(元の手番側)に戻る
    #   scratch_pos.player_disc_count   = 自分の石数
    #   scratch_pos.opponent_disc_count = 相手の石数
    my_cnt  = ctx.scratch_pos.player_disc_count
    opp_cnt = ctx.scratch_pos.opponent_disc_count
    if my_cnt > opp_cnt: return  WIN_SCORE
    if my_cnt < opp_cnt: return -WIN_SCORE
    return 0
...
def order_moves_endgame_with_flips(pos, actions, ctx):
    """
    ① Fastest First (FF) ヒューリスティックによる終盤手の並べ替え。

    「着手後に相手の合法手数が最も少ない手を最初に試す」ことで、
    αβの枝刈りが大幅に増える。フリップ数ベースの並べ替えより
    探索ノード数を数倍〜数十倍削減できることが多い。

    ctx.scratch_pos を一時局面バッファとして使用する。
    order_moves_with_flips と異なり、終盤では EVAL_TABLE よりも
    相手の選択肢を狭める FF の方が枝刈り効果が高い。
    """
    scored = []
    for action in actions:
        flip_bits = pos.calc_flip_discs(action)

        # ① scratch_pos に着手して相手の合法手数を数える
        pos.copy_to(ctx.scratch_pos)
        ctx.scratch_pos.do_move(action, flip_bits)
        opp_moves = sum(1 for _ in ctx.scratch_pos.get_legal_moves())

        # 角への着手はさらに優先 (相手の手数より強いボーナス)
        corner_bonus = -10000 if int(action) in CORNER_SET else 0

        scored.append((corner_bonus + opp_moves, action, flip_bits))

    scored.sort()   # 相手の合法手数が少ない順（昇順）
    return [(a, f) for _, a, f in scored]
...
def endgame_search(pos, alpha, beta, tt_eg, ctx, ply):
    ...
    追加最適化:
        ③ empty_square_count == 1 の場合は _eval_one_empty で直接評価し、
           関数呼び出しオーバーヘッドを排除する。
        ① 手の並べ替えを Fastest First (FF) ヒューリスティックに変更。
    """
    ...
    # EDB ルックアップ: 事前計算済みの勝敗結果があればそのまま返す。
    # endgame_search の再帰の中で何度も参照されるため、
    # _get_bitboards のコストよりも探索省略の効果が大きい。
    if EDB and pos.empty_square_count <= EDB_LOOKUP_LIMIT:
        edb_val = EDB.get(_get_bitboards(pos))
        if edb_val is not None:
            return edb_val

    # ③ 残り1マス: 再帰なし直接評価 (関数呼び出しオーバーヘッドを排除)
    if pos.empty_square_count == 1:
        return _eval_one_empty(pos, ctx, ply)
    ...
    # ① Fastest First で手を並べ替える (ctx を渡す)
...
def alpha_beta_search_only(pos, depth, deadline: float = math.inf,
    ...
    【終盤読み切りの方針】
    探索開始時点 (このルート呼び出し時) に pos.empty_square_count が
    ENDGAME_EMPTY_LIMIT 以下であれば、深さに関係なく即座に endgame_search を
...
def do_move(position: pyrev.Position, time_limit_sec: float, test_mode: bool = True) -> int:
    ...
    for depth in range(1, 64):
        ...
        #
        # 【ケース1】ルート局面が終盤域
        #   pos.empty_square_count <= ENDGAME_EMPTY_LIMIT のとき、
        #   alpha_beta_search_only が endgame_search を直接呼ぶため
        #   depth=1 で完全読みが完了する。
        #
        # 【ケース2】αβ木が全通路を探索済み
        #   alpha_beta_rec 内の終盤遷移を削除した新設計では、αβ木の
        #   完全読みは「depth >= empty_square_count」になった時点で起きる。
        #   それ以上深くしても結果は変わらないためループを終了する。
        #   例: 空きマス13手番で depth=13 以降は全経路が game-over に到達する。

        if position.empty_square_count <= ENDGAME_EMPTY_LIMIT:
            if test_mode:
                ...
                print(f"[do_move] 終盤完全読み完了 (ルート終盤) → 確定 (結果: {result_str})")
            break

        if depth >= position.empty_square_count:
            if test_mode:
                if score is not None and score >= WIN_SCORE:
                    result_str = "自分が勝利"
                elif score is not None and score <= -WIN_SCORE:
                    ...
                    result_str = f"スコア={score}"
                print(f"[do_move] αβ木全探索完了 (depth={depth} >= "
                      f"empty={position.empty_square_count}) → 確定 (結果: {result_str})")
            break
