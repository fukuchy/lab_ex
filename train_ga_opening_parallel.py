"""
GAで ab_rainforce_no_book.py の opening フェーズの評価関数重みだけを学習するスクリプト。

前提:
- このファイルを ab_rainforce_no_book.py と同じディレクトリに置いて実行する。
- ab_rainforce_no_book.py には FEATURE_WEIGHTS と alpha_beta(pos, depth, test_mode) があること。
- 定石ブックは使用しない（ab_rainforce_no_book.py 自体が定石を持たない）。
- middle / end の重みは固定し、opening の7特徴量だけを遺伝子として扱う。

実行例:
    python train_ga_opening_parallel.py

出力:
    ga_opening_best_weights.json
    ga_opening_log.json
"""
...
# opening の初期重みを基準個体にする
BASE_OPENING_WEIGHTS: Dict[str, float] = copy.deepcopy(agent.FEATURE_WEIGHTS["opening"])
...
# 4にすると、黒2手・白2手がランダムに進んだ局面から評価対象の対局が始まる。
...
def set_opening_weights(weights: Dict[str, float]) -> None:
    """
    ab_rainforce_no_book.py 側の opening 重みだけを差し替える。
    middle / end は固定。
    """
    agent.FEATURE_WEIGHTS["opening"] = copy.deepcopy(weights)


def reset_weights() -> None:
    """
    ab_rainforce_no_book.py 側の全重みを初期状態に戻す。
    """
...
def random_individual() -> Individual:
    ...
    for name in FEATURE_NAMES:
        base = BASE_OPENING_WEIGHTS[name]
...
def baseline_individual() -> Individual:
    """
    現在の opening 重みをそのまま使う基準個体。
    毎世代に残しておくと、学習が悪化したか比較しやすい。
    """
    return Individual(weights=copy.deepcopy(BASE_OPENING_WEIGHTS))


def select_agent_move(weights: Dict[str, float], pos: pyrev.Position) -> int:
    """
    指定された opening 重みを agent にセットしてから手を選ぶ。
    """
    set_opening_weights(weights)
...
def play_game(
    ...
    # 序盤 RANDOM_OPENING_PLIES 手をランダムに進める。
    # (黒白合わせてこの手数。例: 4なら黒2手・白2手)
    ...
    for game_index in range(GAMES_PER_INDIVIDUAL):
        ...
        if individual_is_black:
            winner, black_score = play_game(
                ...
                white_weights=BASE_OPENING_WEIGHTS,
            )
            ...
            winner, black_score = play_game(
                black_weights=BASE_OPENING_WEIGHTS,
...
def save_best_weights(best: Individual, filename: str = "ga_opening_best_weights.json") -> None:
    """
    学習後の全フェーズ重みを保存する。
    opening だけ学習済みに差し替え、middle / end は元のまま保存する。
    """
    weights = copy.deepcopy(BASE_ALL_WEIGHTS)
    weights["opening"] = copy.deepcopy(best.weights)

    with open(filename, "w", encoding="utf-8") as f:
        json.dump(weights, f, ensure_ascii=False, indent=4)


def save_log(log: List[dict], filename: str = "ga_opening_log.json") -> None:
    with open(filename, "w", encoding="utf-8") as f:
...
def evaluate_worker(task: Tuple[int, Individual]) -> Tuple[int, Individual]:
    """
    ProcessPoolExecutor から呼び出すためのワーカー関数。

    注意:
    - multiprocessing では関数・引数を pickle できる必要があるため、
      ワーカー関数はトップレベルに定義する。
    - agent.FEATURE_WEIGHTS は各プロセス内で独立して持たれるため、
      select_agent_move() 内で書き換えても他プロセスには影響しない。
    """
    index, individual = task

    # agent 側で乱数を使っている場合に、全プロセスで同じ乱数列にならないようにする。
...
def evaluate_population_parallel(population: List[Individual]) -> List[Individual]:
    """
    集団内の各個体評価をプロセス並列で実行する。
    1個体あたり GAMES_PER_INDIVIDUAL 局のセルフプレイをまとめて1タスクにする。
    """
    max_workers = MAX_WORKERS or os.cpu_count() or 1
    max_workers = max(1, min(max_workers, len(population)))

    evaluated_population: List[Individual] = [None] * len(population)  # type: ignore[list-item]

    tasks = [(i, individual) for i, individual in enumerate(population)]
...
def main() -> None:
    ...
    for generation in range(1, GENERATIONS + 1):
        ...
        evaluated_population = evaluate_population_parallel(population)
        ...
        log.append({
            ...
            "best_opening_weights": copy.deepcopy(best.weights),
        })
    ...
    final_best = global_best if global_best is not None else baseline_individual()
    save_best_weights(final_best)
    save_log(log)
    reset_weights()

    print("\n学習終了")
    print("best weights saved to ga_opening_best_weights.json")
    print("log saved to ga_opening_log.json")
